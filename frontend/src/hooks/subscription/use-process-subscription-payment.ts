...
interface UseProcessSubscriptionPaymentReturnModel {
  processSubscriptionPayment: (payload: SubscriptionPaymentInModel) => Promise<{
    success: boolean;
...
export const useProcessSubscriptionPayment =
  (): UseProcessSubscriptionPaymentReturnModel => {
    const [isLoading, setIsLoading] = useState(false);
    const [error, setError] = useState<string | null>(null);
    const [result, setResult] = useState<SubscriptionPaymentResultModel | null>(
      null
    );
