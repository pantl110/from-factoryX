...
const useUpdateEquipment = () => {
  ...
  const updateEquipment = async (factoryEqId: number, data: Partial<EquipmentResponseModel>) => {
    setIsLoading(true)
    setError(null)
    try {
      const url = `${process.env.NEXT_PUBLIC_API_URL}/v1/factory/equipment/${factoryEqId}`
      ...
      if (response.ok) {
        ...
        const errorMessage =
          errorData.detail || errorData.message || '설비 정보 수정에 실패했습니다.'
