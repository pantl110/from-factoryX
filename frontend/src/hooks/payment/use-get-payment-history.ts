...
const useGetPaymentHistory = () => {
  ...
  const [paymentHistory, setPaymentHistory] = useState<PaymentListResponseModel | null>(null);
  const [error, setError] = useState<string | null>(null);

  const getPaymentHistory = useCallback(async (factoryId: number, page: number = 1, pageSize: number = 10) => {
    if (!factoryId) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorMessage = errorData.detail || '결제 내역을 불러오지 못했습니다.';
