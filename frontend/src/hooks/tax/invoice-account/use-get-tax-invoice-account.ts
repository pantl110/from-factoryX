...
// queryFn을 export하여 useQuery에서 재사용 가능하도록 함
export const getTaxInvoiceAccountQueryFn = async (
  id: number,
  type: 'tax' | 'cash-receipt'
): Promise<TaxInvoiceAccountModel> => {
    ...
    async (id: number, type: 'tax' | 'cash-receipt') => {
      ...
      try {
        const data = await queryClient.fetchQuery<TaxInvoiceAccountModel>({
          ...
          queryFn: () => getTaxInvoiceAccountQueryFn(id, type),
        });
