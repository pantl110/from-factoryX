...
// 타입 정의
export interface BarobillCorpRegisterIn {
  grade: string;
  barobill_id: string;
  barobill_password: string;
  barobill_password_confirm: string;
}
...
// 2. 바로빌 기업 사용자 등록
export const useBarobillCorpUserRegister = () => {
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const factoryId = useFactoryStore((state) => state.factoryId);

  const register = useCallback(async (payload: BarobillCorpRegisterIn) => {
    if (!factoryId) {
      const errorMessage = '공장 ID가 설정되지 않았습니다.';
      setError(errorMessage);
      throw new Error(errorMessage);
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/barobill/register/corp/user`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
        },
        body: JSON.stringify({ factory: factoryId, ...payload }),
      });
