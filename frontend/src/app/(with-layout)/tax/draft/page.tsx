...
const TaxDraftPage = () => {
  const [selectedTab, setSelectedTab] = useState<'전체' | '임시 저장' | '발행 대기'>('전체')
  ...
  // 탭에 따른 필터링된 데이터
  const filteredData =
    selectedTab === '전체' ? tempData : tempData.filter((item) => item.status === selectedTab)
  ...
  return (
    <>
      <div className={`flex flex-col gap-8`}>
        <MainTitleSec selectedTab={selectedTab} setSelectedTab={handleTabChange} />
