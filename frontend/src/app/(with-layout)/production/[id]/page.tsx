...
import completedProjectData, { CompletedProjectDataModel } from '@/mocks/completed-project-data'
...
const getTabsByStatus = (status: string): ProductionTabType[] => {
  ...
  if (status === '프로젝트 완료')
    return ['세금계산서', '거래명세서', '납품', '생산 현황', '생산 내역', '주문서']
  return ['생산 계획', '주문서']
}
...
const ProductionPageContent = () => {
  ...
  const project =
    ...
    completedProjectData.find((item: CompletedProjectDataModel) => item.id === id)
  ...
  const newStatus = project?.status === '완료' ? '프로젝트 완료' : project?.status || null
  ...
  }, [project, isStopped, newStatus, selectedTab, setPageStatus, setProductionTab, tabs])
