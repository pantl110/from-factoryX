...
const PasswordStep = ({ register, errors, watchedValues, isValid, signup }: PasswordStepProps) => {
  return (
    <>
      <div className="flex flex-col">
        ...
        <div className="mt-1 mb-2 h-5">
          {errors.password && <span className="text-red Re_Body-1">{errors.password.message}</span>}
        </div>
      </div>
      <div className="flex flex-col">
        <Input
        ...
        <div className="mt-1 mb-2 h-5">
          {errors.password_confirm && (
            <span className="text-red Re_Body-1">{errors.password_confirm.message}</span>
          )}
          {/* 프론트에서 처리하지 못한 에러 */}
      ...
      <MiniBtn
        ...
        type="submit"
        disabled={
          !isValid || !watchedValues.password || !watchedValues.password_confirm || signup.isLoading
        }
      />
    </>
  )
