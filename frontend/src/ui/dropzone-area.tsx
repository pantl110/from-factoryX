...
import { Image, FilePdf, MicrosoftExcelLogo, File, X } from '@phosphor-icons/react'
...
const DropzoneArea = ({
  ...
  const onDrop = useCallback(
    (acceptedFiles: File[]) => {
      const newFiles = acceptedFiles.filter((file) => !files.some((f) => f.name === file.name)) // 중복된 파일은 제외하고 새로운 파일만 추가
      ...
      {files.length === 0 && (
        <div
          ...
          className={`h-60 rounded-lg border-2 border-dashed border-gr flex flex-col gap-2 justify-center items-center ${
            isDragActive ? 'bg-secondary transition-colors duration-200 border-primary' : ''
          }`}
        >
          {isDragActive ? (
            <>
              {/* 드래그 시 이미지 */}
              <Image className="text-primary w-[68px] h-[73px]" weight="fill" alt="" />
            </>
          ) : (
            <>
              {/* 기본 이미지 */}
              ...
              >
                ...
                <div className="flex justify-between w-full items-center">
                  <div className="flex flex-col">
                    ...
                    <p className="Re_Body-1 text-gr">{getFileTypeInfo(file.type).label}</p>
                  </div>
                  <button
