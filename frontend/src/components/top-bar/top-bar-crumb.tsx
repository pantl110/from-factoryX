...
const TopBarCrumb = ({
  ...
  const isProductionDetail = crumbs[0] === 'production' && crumbs[1] && /^\d+$/.test(crumbs[1])
  ...
  if (crumbs[0] === 'setting') {
    if (settingTab === 'system') {
      ...
      if (settingChip && ['general', 'permission', 'subscription'].includes(settingChip)) {
    ...
    if (index === 0) {
      ...
      if (finalCrumbs[0] === 'project') {
        ...
        if (finalCrumbs[0] === 'stock') {
          ...
          else if (finalCrumbs[1] === '마스터 데이터 관리') tabParam = '?tab=master'
        }
      }
    }
    ...
    <div className="flex items-center gap-1">
      {finalCrumbs.map((crumb, idx) => (
        <div key={idx} className="flex items-center gap-1">
          <p
            className={`Re_Body-1 ${
              isProductionDetail || isQuotation || (finalCrumbs[0] === 'project' && idx === 0)
          ...
          {idx < finalCrumbs.length - 1 && <CaretRight size={16} className="text-[#8c8c8c]" />}
        </div>
      ))}
    </div>
  )
