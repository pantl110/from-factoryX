...
from project.models import ProjectPlanMaterialUsage
from stock.schemas.outbound import MaterialUsageOut
from stock.material_usage_utils import build_material_usage_out, check_plan_permission
        ...
        def process_usages():
            ...
            for item in payload:
                ...
                if item.id:
                    ...
                    has_updates = True
                    try:
                        usage = ProjectPlanMaterialUsage.objects.select_related("plan").get(
                            id=item.id
                        )
                    except ProjectPlanMaterialUsage.DoesNotExist:
                    ...
                    if item.material_repackaging_id is not None:
                        if item.material_repackaging_id == 0:
                            ...
                            try:
                                material_repackaging = MaterialRepackaging.objects.get(
                                    id=item.material_repackaging_id
                                )
                ...
                usage = (
                    ProjectPlanMaterialUsage.objects.select_related(
                        "material",
                        "original_material",
                        "material_history",
...
async def list_plan_material_usages(request, plan_id: int):
    ...
    try:
        ...
        def get_material_usages():
            check_plan_permission(plan_id, int(factory_id))
            usages = (
                ProjectPlanMaterialUsage.objects.filter(plan_id=plan_id)
                .select_related(
                    "material",
                    "original_material",
                    "material_history",
...
async def delete_plan_material_usage(request, usage_id: int):
    ...
    try:
        ...
        def delete_usage():
            ...
            try:
                usage = ProjectPlanMaterialUsage.objects.select_related(
                    "plan__project"
                ).get(id=usage_id)
            except ProjectPlanMaterialUsage.DoesNotExist:
