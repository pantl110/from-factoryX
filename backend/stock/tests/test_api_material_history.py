...
class TestMaterialHistoryAPI(TestCase):
    ...
    async def test_create_material_history_new_materials(self):
        """새로운 원자재들로 이력 생성 테스트"""
        # 원자재 생성 전 개수 확인
        initial_material_count = await sync_to_async(
            Material.objects.filter(factory=self.factory).count
        )()
        ...
        # 첫 번째 원자재 확인
        self.assertEqual(data["materials"][0]["type"], "purchase")
        ...
        material1 = await sync_to_async(Material.objects.get)(
            factory=self.factory, code="NEW001"
        )
        ...
        material2 = await sync_to_async(Material.objects.get)(
            factory=self.factory, code="NEW002"
        )
        ...
        total_materials = await sync_to_async(
            Material.objects.filter(factory=self.factory).count
        )()
        ...
        # 원자재 생성 전후 개수 비교
        self.assertEqual(
            total_materials, initial_material_count + 2
        )  # 2개가 새로 생성됨
    ...
    async def test_create_material_history_mixed_materials(self):
        ...
        new_material = await sync_to_async(Material.objects.get)(
            factory=self.factory, code="NEW003"
        )
    ...
    async def test_material_creation_with_history(self):
        ...
        # 원자재 생성 후 개수 확인
        final_count = await sync_to_async(
            Material.objects.filter(factory=self.factory).count
        )()
        ...
        # 생성된 원자재 확인
        new_material = await sync_to_async(Material.objects.get)(
            factory=self.factory, code="TEST_MATERIAL"
        )
        ...
        history_count = await sync_to_async(
            MaterialHistory.objects.filter(material=new_material).count
        )()
    ...
    async def test_get_material_history_material_not_found(self):
        ...
        headers = await self.authenticate()
        response = await self.client.get(
            f"/?material_id=99999&factory_id={self.factory.id}", headers=headers
        )
        self.assertEqual(response.status_code, 404)
        data = response.json()
        self.assertEqual(
    ...
    async def test_get_material_history_invalid_type_filter(self):
        ...
        # 잘못된 타입으로 조회
        response = await self.client.get(
            f"/?material_id={self.material.id}&factory_id={self.factory.id}&type=잘못된타입",
            headers=headers,
        )
