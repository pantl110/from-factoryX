...
async def get_material_by_id(material_id: int, factory_id: int, user=None):
    ...
    try:
        material = await Material.objects.aget(id=material_id, factory_id=factory_id, factory__owner=user)
        return material
...
async def get_material_history_by_material(material_id: int, factory_id: int, user=None, months=None):
    """원자재의 히스토리를 조회합니다."""
    # 원자재 소유권 검증
...
async def create_material_history(material, client, type, quantity, price=None, user=None):
