...
from tax.utils import create_accounts_for_cash_receipts, TaxInvoiceAccount, AccountStatus
...
async def sync_cash_receipts(request, factory_id: int):
    ...
    if purchase_result.SimpleCashBillExList is not None:
        ...
        # 매입 현금영수증에 대한 account 생성
        await create_accounts_for_cash_receipts(purchase_cash_receipts)
...
@router.get(
    ...
    description="공장, 거래처명(q), 기간, 정렬로 현금영수증을 검색합니다.",
    response=list[AllCashReceiptOut],
)
@paginate
async def list_cash_receipts(
    ...
    await is_factory_member(factory_id, request.auth)
    ...
    def get_filtered_receipts():
        ...
        queryset = queryset.order_by(
            "transaction_date" if order == "asc" else "-transaction_date"
        )

        result = []
        for receipt in queryset.distinct():
            result.append(
                AllCashReceiptOut(
                    id=receipt.id,
                    ...
                    total_amount=(receipt.transaction_amount or 0)
                    + (receipt.tax_amount or 0),
    ...
    return await get_filtered_receipts()
