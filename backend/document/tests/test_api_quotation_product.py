...
class QuotationProductAPITestCase(TestCase):
    ...
    def test_save_draft_quotation_without_client(self):
        ...
        draft_data = {
            "quotation_id": self.quotation.id,
            "products": [
                {"product_id": self.product1.id, "quantity": 20, "unit_price": 1200}
            ],
            # client는 생략
        }
    ...
    def test_save_draft_quotation_existing_client_update(self):
        ...
        self.assertEqual(
            self.client_company.business_registration_number, "987-65-43210"
        )
    ...
    def test_save_draft_quotation_new_client_creation(self):
        ...
        new_client = FactoryClient.objects.get(
            name="새로운 클라이언트", factory=self.factory
        )
        self.assertEqual(new_client.business_registration_number, "111-22-33333")
    ...
    def test_confirm_order_existing_client_update(self):
        ...
        self.assertEqual(
            self.client_company.business_registration_number, "555-44-33333"
        )
    ...
    def test_confirm_order_multiple_products(self):
        ...
        confirmed_data = {
            "quotation_id": self.quotation.id,
            "client": {"name": "다중 제품 고객사"},
            "products": [
                {"product_id": self.product1.id, "quantity": 50, "unit_price": 1000},
                {"product_id": self.product2.id, "quantity": 30, "unit_price": 2000},
            ],
        }
    ...
    def test_confirm_order_missing_client(self):
        ...
        confirmed_data = {
            "quotation_id": self.quotation.id,
            "products": [
                {"product_id": self.product1.id, "quantity": 20, "unit_price": 2000}
            ],
        }
    ...
    def test_confirm_order_without_due_date(self):
        ...
        confirmed_data = {
            ...
            "client": {"name": "테스트"},
            "products": [
                {"product_id": self.product1.id, "quantity": 10, "unit_price": 1000}
            ],
            # due_date 제거
        }
    ...
    def test_confirm_order_with_default_values(self):
        ...
        self.assertEqual(
            plan.quantity, 11
        )  # quotation_product의 quantity + buffer rate
        ...
        self.assertEqual(
            plan.equipment.status, FactoryEquipment.EquipmentStatus.standby
        )  # 가동 대기 상태인 설비만 할당됨
        ...
        # 11개 * 3600초 = 39600초 = 11시간 = 하루 미만
        # 실제 계산된 end_date 사용
        actual_end_date = plan.end_date.date()
        self.assertIsNotNone(actual_end_date)
    ...
    def test_multiple_quotations_same_factory(self):
        ...
        QuotationProduct.objects.create(
            quotation=quotation2, product=self.product1, quantity=7, unit_price=1200
        )
    ...
    def test_quotation_product_filtering(self):
        ...
        delivery_products = QuotationProduct.objects.filter(
            quotation=self.quotation, is_delivery=True
        )
        ...
        non_delivery_products = QuotationProduct.objects.filter(
            quotation=self.quotation, is_delivery=False
        )
    ...
    def test_save_draft_quotation_existing_products_replacement(self):
        """기존 품목 교체 테스트"""
        # 초기 품목 개수 확인
        initial_count = QuotationProduct.objects.filter(
            quotation=self.quotation
        ).count()
        ...
        draft_data = {
            "quotation_id": self.quotation.id,
            "products": [
                {"product_id": self.product1.id, "quantity": 999, "unit_price": 9999}
            ],
        }
    ...
    def test_save_draft_quotation_edge_case_zero_quantity(self):
        ...
        draft_data = {
            "quotation_id": self.quotation.id,
            "products": [
                {"product_id": self.product1.id, "quantity": 0, "unit_price": 1000}
            ],
        }
    ...
    def test_save_draft_quotation_edge_case_zero_price(self):
        ...
        draft_data = {
            "quotation_id": self.quotation.id,
            "products": [
                {"product_id": self.product1.id, "quantity": 10, "unit_price": 0}
            ],
        }
    ...
    def test_save_draft_quotation_malformed_json(self):
        """잘못된 JSON 형식 테스트"""
        malformed_data = (
            '{"quotation_id": 1, "client": {"name": "test"}'  # 닫는 괄호 누락
        )
    ...
    def test_update_quotation_product_delivery_wrong_factory(self):
        ...
        other_quotation = Quotation.objects.create(
            factory=other_factory, client=self.client_company, project=self.project
        )
    ...
    def test_confirm_order_with_production_plans(self):
        ...
        quotation_product = QuotationProduct.objects.create(
            quotation=quotation, product=product, quantity=10, unit_price=1000
        )
    ...
    def test_list_undelivered_quotation_products_pagination(self):
        ...
        # 두 번째 페이지 (2개)
        response = self.client.get(
            f"{url}?factory_id={self.factory.id}&page=2", **self.get_auth_headers()
        )
    ...
    def test_list_undelivered_quotation_products_missing_factory_id(self):
        ...
        response = self.client.get(f"{url}?page=1", **self.get_auth_headers())
