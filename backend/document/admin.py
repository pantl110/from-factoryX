...
class QuotationProductAdmin(admin.ModelAdmin):
    list_display = ['id', 'quotation', 'product', 'quantity', 'unit_price', 'is_delivery', 'delivery_date']
    ...
    fieldsets = (
        ('기본 정보', {
            'fields': ('quotation', 'product', 'quantity', 'unit_price')
        }),
        ('납품 정보', {
            'fields': ('is_delivery', 'delivery_date')
        }),
        ('시스템 정보', {
            'fields': ('created_at', 'updated_at'),
            'classes': ('collapse',)
