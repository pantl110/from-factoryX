...
class TestEmailVerificationFlow(TransactionTestCase):
    ...
    async def test_scenario_1_signup_and_login(self):
        ...
        verification_data = {"email": self.test_email, "verification_type": "signup"}
        response = await self.client.post(
            "/send-verification-code", json=verification_data
        )
    ...
    async def test_scenario_2_signup_password_reset_login(self):
        ...
        verification_data = {"email": test_email, "verification_type": "signup"}
        response = await self.client.post(
            "/send-verification-code", json=verification_data
        )
        self.assertEqual(response.status_code, 200)
        ...
        # 1-2. 인증 코드 확인 및 검증
        verification = await sync_to_async(EmailVerification.objects.get)(
            email=test_email, verification_type="signup", is_verified=False
        )
        ...
        response = await self.client.post(
            "/send-verification-code", json=password_reset_verification_data
        )
        self.assertEqual(response.status_code, 200)

        # 2-2. 비밀번호 재설정용 인증 코드 확인
        reset_verification = await sync_to_async(EmailVerification.objects.get)(
            email=test_email, verification_type="password_reset", is_verified=False
        )
        ...
        response = await self.client.post(
            "/verify-code", json=reset_code_verification_data
        )
        self.assertEqual(response.status_code, 200)
        ...
        old_login_data = {"email": test_email, "password": self.test_password}
...
class TestEmailVerificationEdgeCases(TransactionTestCase):
    ...
    async def test_invalid_email_format(self):
        """잘못된 이메일 형식 테스트"""
        verification_data = {"email": "invalid-email", "verification_type": "signup"}
        response = await self.client.post(
            "/send-verification-code", json=verification_data
        )
        self.assertEqual(response.status_code, 400)
    ...
    async def test_duplicate_email_signup(self):
        ...
        await sync_to_async(User.objects.create_user)(
            username="existing_user", email=self.test_email, password="password123!"
        )
...
class TestLoginEdgeCases(TransactionTestCase):
    ...
    async def test_login_invalid_email_format(self):
        """잘못된 이메일 형식으로 로그인 시도"""
        login_data = {"email": "invalid-email", "password": self.test_password}
        response = await self.client.post("/login", json=login_data)
        self.assertEqual(response.status_code, 400)
        response_data = response.json()
        self.assertIn("올바른 이메일 형식이 아닙니다", response_data["detail"])
    ...
    async def test_login_wrong_password(self):
        ...
        login_data = {"email": self.test_email, "password": "wrong_password"}
        response = await self.client.post("/login", json=login_data)
        self.assertEqual(response.status_code, 400)
        response_data = response.json()
        self.assertIn("비밀번호가 일치하지 않습니다", response_data["detail"])
