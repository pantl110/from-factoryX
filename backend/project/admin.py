...
class ProjectAdmin(admin.ModelAdmin):
    list_display = ['id', 'status', 'transact_date', 'tax_invoice', 'created_at', 'updated_at']
    ...
    fieldsets = (
        ('기본 정보', {
            'fields': ('status', 'transact_date', 'tax_invoice')
        }),
        ('시스템 정보', {
...
class ProjectPlanAdmin(admin.ModelAdmin):
    list_display = ['id', 'project', 'status', 'product', 'quantity', 'equipment', 'start_date', 'end_date']
    ...
    fieldsets = (
        ...
        ('일정 정보', {
            'fields': ('start_date', 'end_date', 'avg_production_time')
        }),
        ('시스템 정보', {
            'fields': ('created_at', 'updated_at'),
            'classes': ('collapse',)
...
class ProjectLogAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('기본 정보', {
            'fields': ('project', 'type', 'title', 'content')
        }),
        ('시스템 정보', {
            'fields': ('created_at', 'updated_at'),
            'classes': ('collapse',)
...
class RefundAdmin(admin.ModelAdmin):
    list_display = ['id', 'project_log', 'product', 'amount', 'refund_date', 'current_stock', 'production_amount']
    list_filter = ['refund_date', 'created_at']
    search_fields = ['project_log__project__id', 'product__name']
    readonly_fields = ['created_at', 'updated_at']
    list_per_page = 20
    
    fieldsets = (
        ('기본 정보', {
            'fields': ('project_log', 'product', 'amount', 'refund_date')
        }),
        ('재고 정보', {
            'fields': ('current_stock', 'production_amount')
        }),
        ('시스템 정보', {
            'fields': ('created_at', 'updated_at'),
            'classes': ('collapse',)
